  ...
  try {
    ...
    let sql = `
      SELECT n.id, n.subject, n.professor, n.category, n.summary,
             n.user_id, u.username AS authorName
      FROM notes n
...
// 수정 폼 (GET /notes/:id/edit) - 작성자만
router.get('/:id/edit', async (req, res, next) => {
  if (!req.session.user) return res.redirect('/login');
  try {
    const [[note]] = await db.promise().query(
      'SELECT id, user_id, subject, professor, category, summary FROM notes WHERE id = ?',
      [req.params.id]
    );
...
router.post('/:id/edit', async (req, res, next) => {
  ...
  try {
    ...
    await db.promise().query(
      'UPDATE notes SET subject=?, professor=?, category=?, summary=? WHERE id=?',
